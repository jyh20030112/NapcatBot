...
""".strip()
    ...
    def build_analysis_task(
        ...
        *,
        ...
        recent_messages = "\n".join(
            _fmt_msg(item) for item in state.recent_messages[-12:] if item.text
        )
        topic_messages = "\n".join(
            _fmt_msg(item) for item in topic.last_messages[-8:] if item.text
        )
...
【该话题最近消息】（按时间顺序，包含你的回复）
{topic_messages or "无"}

【当前群最近消息】（按时间顺序，包含你的回复）
...
{{
  ...
  "risk_level": "normal",
  "confidence": 0.9,
  "analysis": "消息直接@了蛋总，发送者在询问蛋总是否在线。情绪友好，属于闲聊打招呼。话题延续了之前'蛋总去哪了'的讨论。风险正常。"
}}
""".strip()
...
是否回复你的消息: {bool(message.reply_to)}
...
def _fmt_msg(item: BotMessage) -> str:
    return f"{item.nickname}({item.user_id}): {item.text}"
